export const WavRenderer = {
  ...
  normalizeArray: (
    ...
    if (memoize) {
      ...
      cache = WavRenderer.dataMap.has(data) ? WavRenderer.dataMap.get(data) : {};
      WavRenderer.dataMap.set(data, cache);
  ...
  drawBars: (
    ...
    const points = WavRenderer.normalizeArray(data, pointCount, true);
  ...
  /**
   * Renders a circular visualization of audio frequencies
   * @param canvas
   * @param ctx
   * @param data
   * @param color
   * @param lineWidth base width of the line
   * @param maxAmplitude maximum amplitude to scale the line width
   * @param pointCount number of points to render around the circle
   */
  drawCircularFrequencies: (
    canvas: HTMLCanvasElement,
    ctx: CanvasRenderingContext2D,
    data: Float32Array,
    color: string,
    ...
    const normalizedData = WavRenderer.normalizeArray(data, pointCount, true);
    ...
    for (let i = 0; i <= pointCount; i++) {
      ...
      const amplitude = Math.abs(normalizedData[i % pointCount]);
      const scaledRadius = radius + (amplitude / maxAmplitude) * radius * 0.2;
